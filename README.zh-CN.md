...
> 💨 **实测**：69 分钟的视频，音频下载 1 分 18 秒 + AI 转录 1 分 53 秒，全程不到 3 分钟。

### 🔒 数据安全

- **全程本地处理**：音频仅下载到本机临时目录，AI 转录在本机完成——**任何内容都不上传云端**，无遥测、无埋点
- **零账号体系**：无需注册登录，不需要任何 API Key
- **用完即清**：任务结束后临时文件自动删除
- **开源可审计**：全部代码公开，数据处理逻辑可自行审查

### ⚡ 转录速度

下表以一次真实测试为基准（M 系列芯片 Mac、Small 模型：69 分钟视频转录耗时 1 分 53 秒），其余模型按计算量推算，实际取决于设备性能：

| 模型 | 相对实时速度 | 10 分钟视频约需 | 1 小时视频约需 |
|---|---|---|---|
| Tiny | ~100× | ~10 秒 | <1 分钟 |
| Base | ~60× | ~10 秒 | ~1 分钟 |
| Small ⭐ | **~37×（实测）** | ~20 秒 | ~2 分钟 |
| Medium | ~12× | ~50 秒 | ~5 分钟 |
| Large V3 | ~6× | ~1.5 分钟 | ~10 分钟 |

### 📤 导出格式

**TXT**（纯文本 / 带时间戳）、**SRT**（标准字幕文件，可直接导入剪辑软件）、**JSON**（结构化数据，便于二次处理）——一键导出，原生保存对话框。
